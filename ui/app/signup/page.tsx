...
export default function SignUpPage() {
  ...
  useEffect(() => {
    const loadReferenceData = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : signup.errorReferenceData)
      }
    }
  ...
  const handleStep1Submit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.email.trim() || !formData.password) {
      setError(signup.errorStep1Required)
      return
    }

    if (!formData.email.includes('@')) {
      setError(signup.errorInvalidEmail)
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload.detail ?? signup.errorSignupFailed)
      }
      ...
      setError(err instanceof Error ? err.message : signup.errorSignupFailed)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-sm">{signup.subtitle}</p>
        </div>
        ...
        <Card className="border-border">
          {step === 1 && (
            <>
              <CardHeader>
                <CardTitle>{signup.step1Title}</CardTitle>
                <CardDescription>{signup.step1Description}</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleStep1Submit} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="name" className="text-sm font-medium">{signup.fullName}</Label>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-sm font-medium">{signup.emailAddress}</Label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="password" className="text-sm font-medium">{signup.password}</Label>
                    <div className="relative">
                    ...
                    <p className="text-xs text-muted-foreground">{signup.atLeast8Chars}</p>
                  </div>
          ...
          {step === 2 && (
            <>
              <CardHeader>
                <CardTitle>{signup.step2Title}</CardTitle>
                <CardDescription>{signup.step2Description}</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="language" className="text-sm font-medium">{signup.primaryLanguage}</Label>
                      <Select value={formData.languageId} onValueChange={(value) => handleInputChange('languageId', value)}>
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectLanguage} />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="country" className="text-sm font-medium">{signup.countryOfResidence}</Label>
                      <Select value={formData.countryId} onValueChange={(value) => handleInputChange('countryId', value)}>
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectCountry} />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="language-proficiency" className="text-sm font-medium">{signup.languageProficiency}</Label>
                      <Select value={formData.languageProficiency} onValueChange={(value) => handleInputChange('languageProficiency', value)}>
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectProficiency} />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="gender" className="text-sm font-medium">{signup.gender}</Label>
                      <Select value={formData.gender || undefined} onValueChange={(value) => handleInputChange('gender', value)}>
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectGender} />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="age-range" className="text-sm font-medium">{signup.ageRange}</Label>
                      <Select value={formData.ageRange || undefined} onValueChange={(value) => handleInputChange('ageRange', value)}>
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectAgeRange} />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="education-level" className="text-sm font-medium">{signup.educationLevel}</Label>
                      <Select value={formData.educationLevel || undefined} onValueChange={(value) => handleInputChange('educationLevel', value)}>
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectEducationLevel} />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {formData.countryId && filteredDistricts.length > 0 && (
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="space-y-2">
                        <Label htmlFor="district" className="text-sm font-medium">{signup.districtResidence}</Label>
                        <Select value={formData.districtId || undefined} onValueChange={(value) => handleInputChange('districtId', value)}>
                          <SelectTrigger className="border-border">
                            <SelectValue placeholder={signup.selectDistrict} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="tribe-ethnicity" className="text-sm font-medium">{signup.tribeEthnicity}</Label>
                        <Input
                          id="tribe-ethnicity"
                          placeholder={signup.tribeEthnicityPlaceholder}
                  ...
                  <div className="space-y-3 rounded-xl border border-border bg-muted/30 p-4">
                    <Label className="text-sm font-medium">{signup.languageParticipation}</Label>
                    <label className="flex items-start gap-3 rounded-lg border border-border bg-background p-3 cursor-pointer">
                      ...
                      />
                      <span className="text-sm text-muted-foreground">{signup.canRead}</span>
                    </label>
                    <label className="flex items-start gap-3 rounded-lg border border-border bg-background p-3 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm text-muted-foreground">{signup.canRecord}</span>
                    </label>
                    <label className="flex items-start gap-3 rounded-lg border border-border bg-background p-3 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm text-muted-foreground">{signup.canTranscribe}</span>
                    </label>
                    <label className="flex items-start gap-3 rounded-lg border border-border bg-background p-3 cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm text-muted-foreground">{signup.canValidate}</span>
                    </label>
                  </div>

                  <div className="space-y-4 rounded-xl border border-border bg-muted/30 p-4">
                    <div className="space-y-2">
                      <Label htmlFor="speech-impairment" className="text-sm font-medium">{signup.speechImpairment}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger className="border-border">
                          <SelectValue placeholder={signup.selectOption} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="no">{signup.noSpeechImpairment}</SelectItem>
                          <SelectItem value="yes">{signup.hasSpeechImpairment}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    {formData.hasSpeechImpairment && (
                      <>
                        <div className="grid gap-4 lg:grid-cols-2">
                          <div className="space-y-2">
                            <Label htmlFor="speech-condition" className="text-sm font-medium">{signup.condition}</Label>
                            <Select value={formData.speechConditionId} onValueChange={(value) => handleInputChange('speechConditionId', value)}>
                              <SelectTrigger className="border-border">
                                <SelectValue placeholder={signup.selectCondition} />
                              </SelectTrigger>
                              <SelectContent>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="severity" className="text-sm font-medium">{signup.severity}</Label>
                            <Select
                              value={formData.speechConditionSeverity}
                              onValueChange={(value) => handleInputChange('speechConditionSeverity', value)}
                            >
                              <SelectTrigger className="border-border">
                                <SelectValue placeholder={signup.selectSeverity} />
                              </SelectTrigger>
                              <SelectContent>
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="research-consent" className="text-sm font-medium">{signup.researchContact}</Label>
                          <label className="flex items-start gap-3 rounded-lg border border-border bg-background p-3 cursor-pointer">
                            <input
                              type="checkbox"
                              className="mt-1"
                            ...
                            <span className="text-sm text-muted-foreground">
                              {signup.researchContactText}
                            </span>
                          </label>
                        </div>

                        <div className="space-y-2">
                          <Label htmlFor="notes" className="text-sm font-medium">{signup.notes}</Label>
                          <Textarea
                            id="notes"
                  ...
                  <div className="flex flex-col-reverse gap-3 sm:flex-row">
                    ...
                    <Button 
                      ...
                      disabled={isLoading}
                    >
                      {isLoading ? signup.creating : signup.createAccount}
                    </Button>
                  </div>
                </form>
              </CardContent>
          ...
          {step === 3 && (
            <>
              <CardHeader>
                <CardTitle>{signup.welcomeTitle}</CardTitle>
                <CardDescription>{signup.welcomeDescription}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2 text-sm">
                  <p className="text-foreground font-medium">{signup.whatsNext}</p>
                  <ul className="space-y-1 text-muted-foreground text-xs">
                    <li>{signup.nextProfile}</li>
                    <li>{signup.nextVerify}</li>
                    <li>{signup.nextTask}</li>
                    <li>{signup.nextReward}</li>
                  </ul>
                </div>
